...
$sql = call('sql_query', "SELECT b.id, b.board_name, t.replies, b.visible, t.topic_id, t.thread_title, t.latest_reply, p.post_time, p.name_author, p.author_id, p.id, t.views FROM forum_boards AS b LEFT OUTER JOIN forum_topics AS t ON b.id=t.board_id LEFT OUTER JOIN forum_posts AS p ON p.id = t.latest_reply GROUP BY t.topic_id ORDER BY p.post_time DESC LIMIT 10");
if(call('sql_num_rows', $sql) != 0) {
	...
	foreach($fetch as $p) {
		...
		$body .='<tr><td><a href="'.$settings['site_url'].'/index.php?act=viewtopic&amp;id=' . $p['topic_id'] . '">' . $p['thread_title'] . '</a><br /><span class="small-text">Board: <a href="'.$settings['site_url'].'/index.php?act=viewboard&amp;id='.$p['board_id'].'">'.$p['board_name'].'</a></span></td><td align="center">'.$p['replies'].'</td><td align="center">'.$p['views'].'</td><td>'.$post.'</td></tr>';
	}
} else {
