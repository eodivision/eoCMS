...
$(document).ready(function(){
  ...
  if (!isset($_GET['type']) || $_GET['type'] == null) {
      ...
      $body = '<form action="" method="post" id="add"><div class="admin-panel">'.theme('title', 'Add Emoticon').theme('start_content').'<table class="admin-table">
  			<tr>
	...
	<tr class="admin-subtitlebg">
      ...
      while ($p = call('sql_fetch_assoc', $sql)) {
          $body .= '<tr><td>' . $p['code'] . '</td><td><img src="images/emoticons/' . $p['image'] . '" alt="Unavailable"/></td><td>' . $p['alt'] . '</td><td><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=emoticons&amp;type=edit&amp;id=' . $p['id'] . '&amp;'.$authid.'" title="Edit this emoticon"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/listedit.png" alt="Edit" /></a><span class=image-list-spacer>&nbsp;&nbsp;</span><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=emoticons&amp;type=delete&amp;id=' . $p['id'] . '&amp;'.$authid.'" title="Delete this emoticon" onclick="return confirm(\'Are you sure you want to delete this emoticon? All posts using this emoticon will be seen as plain text instead\')"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/delete.png" alt="Delete" /></a><span class=image-list-spacer>&nbsp;&nbsp;</span></td></tr>';
      }
      $body .= '</table>';
  if (isset($_GET['type']) && $_GET['type'] == 'edit') {
      ...
      if ($check == true && !errors()) {
          ...
          session_write_close();
          header("Location: index.php?act=admin&opt=emoticons&".$authid);
      }
      $p = call('sql_fetch_array', call('sql_query', "SELECT * FROM emoticons WHERE id = '" . $_GET['id'] . "'"));
      $body = theme('breadcrumb', array('<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=emoticons&amp;'.$authid.'">Emoticons</a>', $p['alt'])).'<form action="" method="post" id="add"><div class="admin-panel">'.theme('title', 'Edit Emoticon').theme('start_content').'<table class="admin-table">
          ...
          if ($dh = opendir($dir)) {
              while (($file = readdir($dh)) !== false) {
                  if ($file != "." && $file != ".." && $file != 'index.php') {
                      $body .= '<option value="' . $file . '"' . (!errors() && $p['image'] == $file ? 'selected="selected"' : (errors() && $_POST['image'] == $file ? 'selected="selected"' : '')) . '>' . $file . '</option>';
                  }
              }
          }
          closedir($dh);
      }
      $body .= '</select></td></tr><tr><td>Emoticon Preview:</td><td><img src="images/emoticons/' . (errors() ? $_POST['image'] : $p['image']) . '" id="preview" alt="none selected"/></td></tr>
