...
if(!isset($_GET['type']) || $_GET['type']==null) {
	...
	$body ='<form action="#" method="post" id="panels">
	...
	if(call('sql_num_rows', $sql) != 0) {
		while($p = call('sql_fetch_array', $sql)) {
			...
			if(is_array($p['panels']) && count($p['panels']) != 0) {
				$body .= '<optgroup label="'.$p['name'].'">';
				foreach($p['panels'] as $name => $file) {
					$body .= '<option value="Plugins/'.$p['folder'].'/panels/'.$file.'">'.$name.'</option>';	
				}
				$body .= '</optgroup>';
			}
		}
...
<br />';
	...
	foreach($panels as $side => $data) {
		$body.= '<div class="admin-panel2">'.theme('title', $PANELS_LANG['title_'.$side]).theme('start_content').'<table class="admin-table2">';
		$i = 1;
		$count = count($data);
		foreach($data as $panel) {
			...
			if ($i==1 && $count>1) {
				...
				$body .= '<td><a title="' . $PANELS_LANG["t_move_up"] . '" href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=panels&amp;type=order&amp;side='.$panel['side'].'&amp;order=up&amp;id=' . $panel['id'] . '&amp;' . $authid . '"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/orderup.png" alt="' . $PANELS_LANG["t_move_up"] . '" /></a></td>
				<td><a title=" ' . $PANELS_LANG["t_move_down"] . ' " href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=panels&amp;type=order&amp;side=' . $panel['side'] . '&amp;order=down&amp;id=' . $panel['id'] . '&amp;' . $authid . '"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/orderdown.png" alt="' . $PANELS_LANG["t_move_down"] . '" /></a></td>';
			} elseif ($count>1) {
				$body .= '<td><a title="' . $PANELS_LANG["t_move_up"] . '" href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=panels&amp;type=order&amp;side='.$panel['side'].'&amp;order=up&amp;id=' . $panel['id'] . '&amp;' . $authid . '"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/orderup.png" alt="' . $PANELS_LANG["t_move_up"] . '" /></a></td>
				<td></td>';
...
if(isset($_GET['type']) && $_GET['type']=='edit') {
	...
	if($check==true && !errors()) {
		...
		session_write_close();
		header("Location: index.php?act=admin&opt=panels&".$authid);
	}
	$sql = call('sql_query', "SELECT * FROM panels WHERE id = '" . $_GET['id'] . "'");
	$r = call('sql_fetch_assoc', $sql);
	$body = theme('breadcrumb', array('<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=panels&amp;'.$authid.'">Panels</a>', '<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=panels&amp;type=edit&amp;id='.$_GET['id'].'&amp;'.$authid.'">'.$r['panelname'].'</a>')).'<form action="" method="post" id="panels">
<div class="admin-panel">'. theme('title', $PANELS_LANG["title_edit"]) . theme('start_content') . '<table class="admin-table"> 
	...
	<tr>
    	 <td><select name="type">
		<option value="html"'; if (errors() && $_POST['type'] == 'html') { $body .= ' selected="selected"'; } elseif ($r['type']=='html') { $body .= ' selected="selected"'; } $body .= '>HTML</option>
		<option value="php"'; if (errors() && $_POST['type'] == 'php') { $body .= ' selected="selected"'; } elseif ($r['type']=='php') { $body .= ' selected="selected"'; } $body .= '>PHP</option>
	    </select></td>
	</tr>
	<tr>
	  <td>' . $PANELS_LANG["built_in_panels"] . ':</td>
	...
	if (is_dir($dir)){
		...
		<option value="' . $file . '"';
						...
						$body.= '>' . (isset($PANEL_LANG[$file]) ? $PANEL_LANG[$file] : str_replace('_',' ',$file)) . '</option>';
					}
				}
			}
			closedir($dh);
	...
	<tr>
	  <td><select name="online">
		<option value="1" '; if (errors() && $_POST['online']=='1') { $body .= 'selected="selected"'; } elseif ($r['online']=='1') { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["online"] . '</option>
		<option value="0" '; if (errors() && $_POST['online']=='0') { $body .= 'selected="selected"'; } elseif ($r['online']=='0') { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["offline"] . '</option>
	    </select></td>
	</tr>
	<tr>
	  <td>' . $PANELS_LANG["panel_placement"] . ':</td>
	...
	<tr>
	  <td><select name="side">
		<option value="left" ';  if (errors() && $_POST['side']=='left')  { $body .= 'selected="selected"'; } elseif ($r['side']=='left')  { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["left"] . '</option>
		<option value="right" '; if (errors() && $_POST['side']=='right') { $body .= 'selected="selected"'; } elseif ($r['side']=='right') { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["right"] . '</option>
		<option value="upper" '; if (errors() && $_POST['side']=='upper') { $body .= 'selected="selected"'; } elseif ($r['side']=='upper') { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["upper"] . '</option>
		<option value="lower" '; if (errors() && $_POST['side']=='lower') { $body .= 'selected="selected"'; } elseif ($r['side']=='lower') { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["lower"] . '</option>
	    </select></td>
	</tr>
	<tr>
	  <td>' . $PANELS_LANG["display_on_all_pages"] . '? (' . $PANELS_LANG["on_page_id_1_only"] . ')</td>
	...
	<tr>
	  <td><select name="all">
		<option value="0" '; if($check==false && errors() && $_POST['all']=='0') { $body .= 'selected="selected"'; } elseif ($r['all_pages']=='0') { $body .= 'selected="selected"'; } $body.='>' . $PANELS_LANG["no"] . '</option>
		<option value="1" '; if($check==false && errors() && $_POST['all']=='1') { $body .= 'selected="selected"'; } elseif ($r['all_pages']=='1') { $body .= 'selected="selected"'; } $body .= '>' . $PANELS_LANG["yes"] . '</option>
	    </select></td>
	</tr>
	<tr>
