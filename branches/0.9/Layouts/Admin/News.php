	  ...
	  <td  colspan="2" align="center"><input type="submit" value="Add News" name="submit" /><input name="Reset" type="reset" value="Reset" /></td>
		...
		if(call('sql_num_rows', $sql) == 0)
			...
			foreach ($news as $p) {
				...
				<td width="10%"><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=news&amp;type=edit&amp;id=' . $p['id'] . '&amp;'.$authid.'" title="Edit"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/listedit.png" alt="Edit" /></a><span class="image-list-spacer">&nbsp;&nbsp;</span><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=news&amp;type=delete&amp;id=' . $p['id'] . '&amp;'.$authid.'" title="Delete" onclick="return confirm(\'Are you sure you want to delete this news?\')"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/delete.png" alt="Delete" /></a><span class="image-list-spacer">&nbsp;&nbsp;</span></td>
			  </tr>';
			}
		}
...
if(isset($_GET['type']) && $_GET['type'] == 'edit') {
	...
	if($check==true && !errors()) {
		...
		session_write_close();
		header("Location: index.php?act=admin&opt=news&".$authid);
	}
...
if(isset($_GET['type']) && $_GET['type'] == 'category') {
	...
	if($check==true && !errors()) {
		...
		session_write_close();
		header("Location: index.php?act=admin&opt=news&type=category&".$authid);
	} 
