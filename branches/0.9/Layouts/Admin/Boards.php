...
$catcount = call('sql_num_rows', $query);
          if ($catcount == 0) {
              ...
              foreach ($cats as $cat) {
                  $i = 1;
                  $body .= '<tr class="admin-subtitlebg"><td>' . $cat['cat_name'] . '</td>';
			...
			<td><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=forum&amp;type=deletecat&amp;id=' . $cat['cat_id'] . '&amp;' . $authid . '" title="'.$BOARDS_LANG["t_delete"].'" onclick="return confirm(\''.$BOARDS_LANG["js_delete"].'\')"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/delete.png" alt="'.$BOARDS_LANG["t_delete"].'" /></a></td>';
			if ($j == 1) {
			...
			<span class="small-text">' . $p['board_description'] . '</span></td>';
					  ...
					  <td><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=forum&amp;type=delete&amp;id=' . $p['id'] . '&amp;' . $authid . '" title="'.$BOARDS_LANG["t_delete"].'" onclick="return confirm(\'Are you sure you want to delete this board?\')"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/delete.png" alt="'.$BOARDS_LANG["t_delete"].'" /></a></td>';
  ...
  if (isset($_GET['type']) && $_GET['type'] == 'edit') {
      ...
      if ($check == true && !errors()) {
          ...
          session_write_close();
          header("Location: index.php?act=admin&opt=forum&" . $authid);
      }
