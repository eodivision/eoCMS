...
switch ($_GET['opt']) {
	...
	case 'deletepost':
		...
		if($check==true) {
			header("Location: index.php?act=viewtopic&id=" . $_GET['topicid']);
		}
		break;
	case 'deletetopic':
		$check = call('delete_topic', $_GET['id'], $_GET['board_id']);
		if($check==true) {
			header("Location: index.php?act=viewboard&id=" . $_GET['board_id']);
		} else {
	...
	case 'locktopic':
		...
		if($check==true) {
			header("Location: index.php?act=viewtopic&id=" . $_GET['id']);
		}
		break;
	case 'unlocktopic':
...
$(document).ready(function(){
	$("#redirect").click(function(){
		...
		$fetch = call('sql_fetch_array',call('sql_query', "SELECT thread_title FROM forum_topics WHERE topic_id='" . $_GET['id'] . "'"));
  ...
  <tr id="redirect-topic">
    ...
    <td><input type="text" name="title" maxlength="80" value="';
		if(errors()) {
			...
			$body .= $MODERATORCP_LANG["moved"].': ' .$fetch['thread_title'];
		}
		$body .='" /></td>
  </tr>
...
<p align="center"><input type="submit" value="'.$MODERATORCP_LANG["btn_move_topic"].'" /></p>
	...
	case 'deletecomment':
		...
		if($check==true) {
			header("Location: index.php?act=news&readmore=" . $_GET['readmore']);
		}
		break;
	case 'quick':
		if($user['delete_any_topic']) {
			...
			header("Location: index.php?act=viewtopic&id=".$_GET['topic']);
			die();
			}
				...
				while($p = call('sql_fetch_assoc', $sql)) {
					if(is_array($_POST['posts']) && in_array($p['id'], $_POST['posts'])) {
						$ip[] = $p['ip'];
						$members[$p['author_id']] = $p['name_author'];
						$board_id = $p['board_id'];
				...
				if(isset($_POST['submit'])) {
					...
					header("Location: index.php?act=viewboard&id=".$_POST['board']);
				} else {
