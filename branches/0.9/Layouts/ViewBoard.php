...
call('getboardpermission', $_GET['id']);
...
if($moderator==true && isset($_POST['topics'])) {
	...
	header("Location: ".$settings['site_url']."/index.php?act=viewboard&id=".$_GET['id']);
}
...
if ($fetch!=false) {
  ...
  <tr class="'.($r['sticky'] == 1 ? 'sticky' : 'topic').'" id="topic_' . $r['topic_id'] . '">
	...
	elseif ($r['replies']>='15') {
		...
		$body .= '</td>
    <td align="left" title="'.$r['message'].'"'.($moderator==true ? '  onclick="edit_title(\''.$r['topic_id'].'\')"' :'').'><div id="topictitle_'.$r['topic_id'].'"><strong><a href="'.$settings['site_url'].'/index.php?act=viewtopic&amp;id=' . $r['topic_id'] . '">' . $r['thread_title'] . '</a></strong>'.($r['read_topic_id'] == null ? ' <span class="new">NEW</span>' : '').($replies>1 ? ' <span class="small-text">'.$VIEWBOARD_LANG["pages"].': ' . trim($pages) . '</span>' : '').'</div>
	<div class="small-text">' . (!empty($r['thread_author']) ? call('userprofilelink', $r['thread_author']) : $VIEWBOARD_LANG["guest"]) . '</div>';
		$body .= '</td>
