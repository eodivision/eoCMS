...
if(isset($settings['seo_links']) && $settings['seo_links']) {
	...
	function topic_url($url) {
		if(is_numeric($url[2])) {
			$sql = call('sql_query', "SELECT topic_id, thread_title FROM forum_topics", 'cache');
			$count = count($sql) - 1;
			for($i = 0; $i <= $count; $i++) {
				if($sql[$i]['topic_id'] == $url[2]) {
					return 'forum/'.clean_url($sql[$i]['thread_title']).'.'.$url[2].(isset($url[5]) && $url[5] > 1 ? '.p'.$url[5] : '').'.html'.(isset($url[6]) ? $url[6] : '');
					break;
				}
			}
		}
