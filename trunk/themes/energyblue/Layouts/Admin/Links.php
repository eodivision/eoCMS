...
$body.= '<option value="' . $pagefetch['id'] . '"'; if($check == false  && isset($_POST['predefined']) && $_POST['predefined'] == $pagefetch['id']) { $body.= 'selected="selected"'; } $body.='>' . $pagefetch['pagename'] . '</option>';
  ...
  <td  colspan="2" align="center"><input type="submit" value="Add Link" name="Submit" /><input name="Reset" type="reset" value="Reset" /></td>
        ...
        foreach ($links as $p)
        {
            ...
            <td width="10%"><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=links&amp;type=edit&amp;id=' . $p['id'] . '&amp;'.$authid.'" title="Edit"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/listedit.png" alt="Edit" /></a><span class="image-list-spacer">&nbsp;&nbsp;</span><a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=links&amp;type=delete&amp;id=' . $p['id'] . '&amp;'.$authid.'" title="Delete" class="delete"><img src="'.$settings['site_url'].'/themes/' . $settings['site_theme'] . '/images/delete.png" alt="Delete" /></a><span class="image-list-spacer">&nbsp;&nbsp;</span></td>
          </tr>';
        }
...
session_write_close();
call('redirect', 'index.php?act=admin&opt=links&'.$authid);
}
