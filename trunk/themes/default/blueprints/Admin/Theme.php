...
$theme['body'] = '<form action="" method="post"><div class="admin-panel">'.theme('title', $THEME_LANG["theme_settings"]).theme('start_content').'<table class="admin-table">
  <tr>
	...
	$theme['body'].='</select> <a id="site_preview_reset" href="">' . $THEME_LANG["preview"] . '</a></td>
	</tr>
	<tr>
...
if(isset($_GET['type']) && $_GET['type']=='install') {
	...
	$theme['body'] = theme('breadcrumb', array('<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=themes&amp;'.$authid.'">Themes</a>', 'Install', $theme['name'] )).'<form action="" method="post"><div class="admin-panel">'.theme('title', $THEME_LANG["install_theme"].' "'.$theme['name'].'"').theme('start_content').'<table class="admin-table">
	<tr>
	...
	while ($m = call('sql_fetch_array', $sql2)) {
		...
		<input type="checkbox" value="' . $m['membergroup_id'] . '" name="visible[]" id="themes_' . $m['membergroup_id'] . '"';
			...
			$theme['body'] .= '/></td><td><label for="themes_' . $m['membergroup_id'] . '">' . $m['name'] . '</label></td></tr>';
		}
	}
	$theme['body'] .= '</td>
	</tr>
...
if(isset($_GET['type']) && $_GET['type']=='edit') {
	...
	$theme['body'] = theme('breadcrumb', array('<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=themes&amp;'.$authid.'">Themes</a>', $theme['name'] )).'<form action="" method="post"><div class="admin-panel">'.theme('title', '"' . $theme['name'] . '" ' . $THEME_LANG["edit_theme_settings"]).theme('start_content').'<table class="admin-table2">
  <tr>
	...
	while ($m = call('sql_fetch_array', $sql2)) {
		...
		<input type="checkbox" value="' . $m['membergroup_id'] . '" name="visible[]" id="topics_' . $m['membergroup_id'] . '"';
			...
			$theme['body'] .= '/><label for="topics_' . $m['membergroup_id'] . '">' . $m['name'] . '</label><br />';
		}
	}
	$theme['body'] .= '</td>
	</tr>
...
# uh oh looks like it is, someone has been playing with the URLs
	...
	$theme['body'] = '<form action="" method="post">
