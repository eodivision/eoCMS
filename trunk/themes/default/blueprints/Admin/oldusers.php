...
$theme['title'] = 'Users';
$theme['head'] = '<script type="text/javascript" src="' . $settings['site_url'] . '/js/jquery.validate.js"></script><script type="text/javascript" src="' . $settings['site_url'] . '/js/jquery.populate.pack.js"></script><script type="text/javascript">
...
if(!isset($_GET['type']) || $_GET['type'] == null) {
	...
	$theme['body'] = $pagination . '<form onsubmit="return false;" action="" name="order"><div class="admin-panel2">'.theme('title', 'Current Users').theme('start_content').'
<table class="admin-table2 users">
	...
	$theme['body'] .='</table>'.theme('end_content').'</div>' . $pagination . '<div style="text-align: right"><select name="checked" id="checked"><option value="">With Selected...</option><option value="delete">Delete</option><option value="ban">Ban IPs</option><option value="nologin">Prevent Login</option></select></form></div>';
}
if(isset($_GET['type']) && $_GET['type'] == 'edit') {
	...
	$theme['head'] .= '<script type="text/javascript">function textCounter(field, countfield, maxlimit) {
...
countfield.value = maxlimit - field.value.length;
	...
	$theme['body'] = theme('breadcrumb', array('<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=users&amp;'.$authid.'">Members</a>', '<a href="'.$settings['site_url'].'/index.php?act=admin&amp;opt=users&amp;type=edit&amp;id='.$_GET['id'].'&amp;'.$authid.'">'.$r['user'].'</a>')).'<form action="" method="post" id="users"><div class="admin-panel">'.theme('title', 'Edit User').theme('start_content').'<table class="admin-table2">
  <tr>
  	<td>Username:</td>
    <td><input type="text" name="username" class="required" value="'; if(errors()) { $theme['body'].= $_POST['user']; } else { $theme['body'] .= $r['user']; } $theme['body'] .='" /></td>
  </tr>
  <tr>
  ...
  <tr>
  	<td>Email:</td>
    <td><input type="text" name="email" class="required email" value="'; if(errors()) { $theme['body'].= $_POST['email']; } else { $theme['body'] .= $r['email']; } $theme['body'] .='" /></td>
  </tr>
  <tr>
  	<td>Membergroup:</td>
	...
	while($m = call('sql_fetch_array',$sql2)) {
       $theme['body'].='<option value="' . $m['membergroup_id'] . '"'; if($m['membergroup_id'] == $r['membergroup']) { $theme['body'].=' selected="selected"'; } $theme['body'].='>' . $m['name'] . '</option>';
	}
      $theme['body'].='</select>
    </td>
	</tr>
  <tr>
  	<td>New Password:</td>
    <td><input type="password" name="pass" id="password" value="'; if(errors()) { $theme['body'].= $_POST['pass']; } $theme['body'] .='" />
    </td>
  </tr>
  <tr>
  	<td>Confirm New Password:</td>
    <td><input type="password" name="newpass" id="password_confirm" value="'; if(errors()) { $theme['body'].= $_POST['newpass']; } $theme['body'] .='" />
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>'; if(!empty($r['avatar'])) { $theme['body'].= '<img src="' . $r['avatar'] . '" alt="Avatar" />'; } else { $theme['body'] .= 'None'; } $theme['body'] .='</td>
  </tr>
  <tr><td>
  Url to Avatar:
<br/>
...
<td><input type="text" name="avatar" class="url" value="'; if(errors()) { $theme['body'].= $_POST['avatar']; } else { $theme['body'] .= $r['avatar']; } $theme['body'] .='" /></td>
</tr><tr><td>Signature:</td><td><textarea name="signature" id="signature" cols="50" rows="3" onKeyDown="textCounter(this.form.signature,this.form.characters,300);" onKeyUp="textCounter(this.form.signature,this.form.characters,300);" style="width: 100%;">'; if(errors()) { $theme['body'].= $_POST['signature']; } else { $theme['body'] .= $r['signature']; } $theme['body'] .='</textarea><br>Characters Left:<input readonly type=text name=characters size="3" maxlength="3" value="300" /></td></tr>
	  <tr><td>Location:</td>
	  <td><input type="text" name="location" value="'; if(errors()) { $theme['body'].= $_POST['location']; } else { $theme['body'] .= $r['location']; } $theme['body'] .='" /></td>
	  </tr>
