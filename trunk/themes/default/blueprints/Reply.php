...
if(isset($_GET['quote'])) {
$theme['body'] = '<body onLoad="InsertQuote(\'' . $_GET['quote'] . '\', \'reply\', \'\', \'post\');">';
}
else { $theme['body'] =''; }
$theme['body'] .= theme('title', $REPLY_LANG["theme_title"]) . theme('start_content') . '
				<form action="" method="post" id="reply"><table class="reply">
  <tr>
    <td>'.$REPLY_LANG["subject"].':</td>
    <td><input type="text" name="subject" id="subject" size="60" maxlength="84" value="'; if(isset($_POST['topic_title'])) { $theme['body'].=$_POST['topic_title']; } else { $theme['body'].='Re: ' . $topicfetch['topic_title']; } $theme['body'].='" /></td>
  </tr>
  <tr>
    <td>'.$REPLY_LANG["message"].':</td>
    <td>' . call('bbcodeform', 'message') . '<textarea rows="12" name="message" id="message" class="required">'; if(isset($_POST['message'])) { $theme['body'].=$_POST['message']; } $theme['body'].='</textarea></td>
  </tr>
...
$theme['body'] .= theme('title', $REPLY_LANG["topic_summary"]) . theme('start_content');
while($fetch = call('sql_fetch_array', $sql)){
$theme['body'] .= '<table align="center" style="table-layout: fixed;" width="100%">
							<tr>
