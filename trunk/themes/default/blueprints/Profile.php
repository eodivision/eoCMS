...
if(!$user['guest'] && $fetch['user']==$user['user'])
	$theme['title'] = $PROFILE_LANG["theme_title"];
else
	$theme['title'] = $PROFILE_LANG["title"] . ' ' . $fetch['user'];
...
$theme['body'] = theme('title', $theme['title']) .  theme('start_content').'<table id="profile" width="100%" class="profile-area">
<tr>
	<td class="profile-avatar-contact" width="10%" valign="top"><div class="profile-username">'.$fetch['user'].'</div>
		...
		<td class="profile-details-area">
			...
			if($fetch['gender']) {
				$theme['body'].='<div class="profile-details profile-titles profile-info-gender">'.$PROFILE_LANG["gender"].':</div>
...
if(!empty($fetch['signature'])) {
	$theme['body'].= theme('title', $PROFILE_LANG["theme_title_sig"]).theme('start_content').'<div id="profileSig" class="profile-sig" style="overflow: auto;">' . call('bbcode', $fetch['signature']) . '</div>'.theme('end_content');
}
} if(isset($_GET['opt']) && $_GET['opt'] == 'showposts') {
$fetch = call('displayuserinfo', (isset($_GET['id']) ? $_GET['id'] : $user['id']));
$theme['title'] = $PROFILE_LANG['latest_posts_of'].': '.$fetch['user'];
$theme['body'] = theme('title', $theme['title']).theme('start_content');
$posts = call('showposts', $fetch['id']);
$theme['body'] .='<table width="100%"style="table-layout: fixed;">';
...
$theme['body'] .='<tr class="subtitlebg">
...
$theme['body'] .='<tr><td>'.$pagination.'</td></tr></table>'.theme('end_content');
}
?>