...
if($check==true && !errors()) {
	call('redirect', 'index.php?act=pm');
} else {
	if($_GET['pm']!='') {
		$query = call('sql_query', "SELECT * FROM pm WHERE id='" . $_GET['pm'] . "'");
		$fetch = call('sql_fetch_array',$query);
	}
	$theme['head'] = '<script type="text/javascript" src="' . $settings['site_url'] . '/js/jquery.validate.js"></script><script type="text/javascript" src="' . $settings['site_url'] . '/js/jquery.populate.pack.js"></script>
...
$(document).ready(function() {
	...
	$theme['body'] = theme('title', $SENDPM_LANG["theme_title"]) . theme('start_content') . '
<form action="" method="post" id="sendpm"><table>
  ...
  <tr>
    ...
    <td><input type="text" name="to" class="required" id="to"'; if(isset($_GET['userid'])) { $fetch2 = call('displayuserinfo', $_GET['userid']); $theme['body'] .= ' value="' . $fetch2['user'] . '">'; } elseif(isset($_POST['to'])) {  $theme['body'] .= ' value="' . $_POST['to'] . '">'; } $theme['body'] .= ' <a href="javascript:;" onclick="window.open(\''.$settings['site_url'].'/index.php?act=finduser&area=to\',\'\',\'width=400,height=300\')" class="help"></a></td>
  </tr>
  <tr>
    <td>'.$SENDPM_LANG["subject"].':</td>
    <td><input type="text" name="title" class="input" id="title" maxlength="80" value="'; if($_GET['pm']!='') { $theme['body'].= (!preg_match("/RE/", $fetch['title'])? 'RE:' . $fetch['title'] : $fetch['title']); } elseif(isset($_POST['title'])) { $theme['body'].= $_POST['title']; } $theme['body'].='" class="required"></td>
  </tr>
  <tr>
    <td>'.$SENDPM_LANG["message"].':</td>
    <td>'.call('form_token').call('bbcodeform', 'message').'<textarea rows="12" cols="40" name="message" id="message" type="text" class="required">'; if(isset($_POST['message'])) $theme['body'].= $_POST['message']; $theme['body'].='</textarea></td>
  </tr>
</table>
