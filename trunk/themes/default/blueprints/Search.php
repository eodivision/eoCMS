...
if(!(defined("IN_ECMS"))) die("Hacking Attempt...");
  ...
  $theme['title'] = $SEARCH_LANG["title"];
  $pageNum = (isset($_GET['page']) ? $_GET['page'] : 1);
  // how many rows to show per page
  $rowsPerPage = 20;
  // counting the offset
  $offset = ($pageNum - 1) * $rowsPerPage;
  $theme['body'] = theme('title', $SEARCH_LANG["theme_title"]) . theme('start_content');
  $theme['body'] .= '<form method="get" action=""><table>
...
<tr><td align="center" colspan="2"><input type="hidden" name="act" value="search" /><input type="submit" value="'.$SEARCH_LANG["btn_search"].'" /></td></tr></table></form>';
  ...
  //theres a search so lets show the table
      $theme['body'] .= theme('title', $SEARCH_LANG["theme_title_results"]) . theme('start_content');
      $sqlnum = call('sql_num_rows', $sql);
      if ($sqlnum == 0) {
	  //no results, hate it when that happens
          $theme['body'] .= $SEARCH_LANG["body_content"].'<br />';
      } else {
	  //there are results lets show them
          $theme['body'] .= $sqlnum . ' '.$SEARCH_LANG["results_found"].': ' . $_GET['search'] . '<br />'.$results;
      }
	  //check to make sure there are results
      if ($sqlnum != 0) {
	  //display the pages
          $theme['body'] .= (isset($_GET['search']) ? $pagination : '');
      }
  }
  $theme['body'] .= theme('end_content');
?>