...
switch ($_GET['opt']) {
	...
	case 'move':
		...
		$theme['title'] = $MODERATORCP_LANG["title"];
...
$(document).ready(function(){
	$("#redirect").click(function(){
		...
		$theme['body'] = theme('title', $MODERATORCP_LANG["theme_title"]) . theme('start_content') . '
<form action="" method="post" id="editpost"><table>
  ...
  <tr id="redirect-topic">
    ...
    <td><input type="text" name="title" maxlength="80" value="';
		if(errors()) {
			...
			$theme['body'] .= $MODERATORCP_LANG["moved"].': ' .$fetch['topic_title'];
		}
		$theme['body'] .='" /></td>
  </tr>
  <tr id="redirect-topic2">
    ...
    <td><textarea rows="12" cols="40" name="message" id="message">';
		if(errors()) {
			$theme['body'] .= $_POST['message'];
		} else {
			$theme['body'] .= $MODERATORCP_LANG["topic_moved"]." [BOARD] \n\n [TOPIC LINK]";
		}
		...
		if($user['delete_any_topic']) {
			if(isset($_POST['opt']) && $_POST['opt']=='spam') {
				...
				if(isset($_POST['submit'])) {
					...
					$theme['title'] = $MODERATORCP_LANG["title_spam"];
					$theme['body'] = theme('title', $MODERATORCP_LANG["theme_title_spam"]).theme('start_content').'
...
<input type="hidden" name="opt" value="spam" /><input type="hidden" name="posts" value="';
					...
					if(is_array($_POST['posts'])) {
						...
						$theme['body'] .= $_POST['posts'];
					}
					$theme['body'].='" /><input type="hidden" name="board" value="' . $board_id . '" /><input type="submit" value="'.$MODERATORCP_LANG["btn_delete_spam"].'" name="submit" />
</form>';
