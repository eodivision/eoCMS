...
function newtopic($board, $title, $message, $token, $smiley) {
	...
	if(!$user['admin_panel'])  {
		$c = call('sql_query', "SELECT date_created, topic_author from forum_topics WHERE topic_ip = '".call('visitor_ip')."'");
		while ($c2 = call('sql_fetch_array', $c)) {
			$difference = time() - $c2['date_created'];
			if ($difference < 100) {
				//Spammers!
				$error[] = $c2['topic_author'].', You have already posted, please wait for a few minutes and try again later';
				return false;
			}
		}
	}
	if(!errors()) {
		...
		$insertpost = call('sql_query', "INSERT INTO forum_topics (board_id, topic_title, topic_author, topic_ip, date_created".$creationtable.") VALUES ('$board', '$title', '" . $user['user'] . "', '" . call('visitor_ip') . "', '" . time() . "'".$creation.")");
