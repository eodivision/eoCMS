...
function deletecat($id) {
	...
	$sql = call('sql_query', "SELECT * FROM news_categories WHERE id = '$id'");
	if(call('sql_num_rows', $sql) == 0) {
		$error[] = 'This category no longer exists';
		return false;
	}
	if(!errors()) {
		$query = call('sql_query', "DELETE FROM news WHERE cat = '$id'");
		$query_2 = call('sql_query', "DELETE FROM news_categories WHERE id = '$id'");
		if($query)
			return true;
	}
}
