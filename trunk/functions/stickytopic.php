...
*/
function sticky_topics($topicarray) {
	...
	$checktopic = call('sql_query', "SELECT topic_author FROM forum_topics WHERE topic_id IN ($topics)");
	...
	if(($fetch['topic_author'] == $user['user'] && $user['sticky_own_topic']) || ($user['sticky_any_topic'])) {
    	if(!errors()) {
			$query = call('sql_query', "UPDATE forum_topics SET sticky = '1' WHERE topic_id IN ($topics)");
			if($query)
				return true;
		}
	} else {
