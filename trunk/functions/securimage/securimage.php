...
class Securimage {
	...
	*/
	...
	function Securimage()
	{
		...
		$this->wordlist_file = IN_PATH.'functions/securimage/words/words.txt';
		$this->use_wordlist  = false;

		$this->gd_font_file  = IN_PATH.'functions/securimage/gdfonts/automatic.gdf';
		$this->use_gd_font   = false;
		$this->gd_font_size  = 24;
		$this->text_x_start  = 15;

		$this->ttf_file      = IN_PATH.'functions/securimage/AHGBold.ttf';
	...
	function createCode()
	{
		...
		if($this->code == false && isset($_GET['code'])) {
			$sql = call('sql_query', "SELECT captcha_code FROM csrf_forms WHERE captcha_encode = '".$_GET['code']."'");
			if(call('sql_num_rows', $sql) == 0)
				die('error');
			list($captcha_code) = call('sql_fetch_row', $sql);
			$this->code = $captcha_code;
			return $this->code;
		}
		// This is the refresh
		if($this->code == false && isset($_GET['token'])) {
			$code = '';
			$code_length = 6;
			...
			call('sql_query', "UPDATE csrf_forms SET captcha_code = '$code', captcha_encode = '$encode' WHERE token = '".$_GET['token']."'");
			$this->code = $code;
			return $code;
		}
	}
	...
	function validate()
	{
		$code_entered = trim(strtolower($_POST['captcha']));
		$sql = call('sql_query', "SELECT captcha_code FROM csrf_forms WHERE captcha_code = '$code_entered'");
		if(call('sql_num_rows', $sql) != 0) {
			call('sql_query', "DELETE FROM csrf_forms WHERE captcha_code = '$code_entered'");
			return true;
		} else
			return false;
	}
