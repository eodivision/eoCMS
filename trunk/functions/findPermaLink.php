...
function findPermaLink($permalink){
	...
	$sql = call('sql_query',"SELECT topic_id, topic_title FROM forum_topics WHERE topic_title LIKE '". $words[0] ."%'");
	if(!$sql) return false;
	...
	for($i=0;$i<$row_count;$i++){
		...
		foreach($words as $word){
			if(strpos($fetch['topic_title'],$word)===false) { $found = false; break; }
		}
