...
$(document).ready(function() {
	$("#list").tableDnD({
		...
		var neworder = $.tableDnD.serialize();
            $.post("index.php?act=ajax&m=plugin&id='.$_GET['id'].'", { order: neworder }, function(data, textStatus) { if(textStatus == "success") { $("#update").text("New order saved").slideDown(); } else { $("#update").text("Unabled to save new order").slideDown(); }}, "text");
	    }
	});
	$("#links").validate();
...
session_write_close();
call('redirect', 'index.php?act=admin&opt=plugins&type=admin&id='.$_GET['id'].'&'.$authid);
}
...
$check = plugin('deletelink', 'navpanel', $_GET['linkid']);
if($check==true && !errors()) {
